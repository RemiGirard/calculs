import { MouseEventHandler, UIEventHandler, useCallback, useEffect, useState } from "react";
...
const useScrollBar = ({windowRef, contentRef, scrollTrackRef, direction = "vertical"}
  ...
  const [scrollContentPercentage, setScrollContentPercentage] = useState<number>(0);
  ...
  const scrollEvent:UIEventHandler<HTMLDivElement> = (e) => {
    ...
    setScrollContentPercentage((target[wOrH.scrollRect] / (contentRef.current !== null && contentRef.current[wOrH.scroll] ? contentRef.current[wOrH.scroll] : 1)) * 100);
    // contentRef.current?.scrollHeight
  }
    return {handleThumbMousedown, handleTrackClick, handleScrollBarSizeChange, scrollEvent, scrollBarSize, scrollContentPercentage};
};
