...
export const eachDigitAdditionIsInferiorToTen = (numbers: number[]): boolean => {
  ...
  return number1String.split('').every((digit, index) => parseInt(digit) + otherNumbersString.map((otherNumberString) => parseInt(otherNumberString[index])).reduce((a, b) => a + b) < 10);
};
...
export const eachDigitSubtractionIsPositive = (number1: number, number2: number): boolean => {
  ...
  return number1String.split('').every((digit, index) => parseInt(digit) - parseInt(number2String[index]) >= 0);
};
