...
export default ({exercise, setExercise, deleteExercise, canBeDeleted}: componentProps) => {
    ...
    return (<ExerciseConfigWrapper>
        ...
        <div>
          {exercise.columnList.map((column, index) => {
            ...
            return <ColumnConfigWrapper key={index}>
                <InputColumnWrapper>
        ...
        <div>{JSON.stringify(exercise.columnList.map((column)=>{
            return column.equations;
        }))}</div>
